...
class _GetContactPageState extends State<GetContactPage> {
  ...
  inputPhoneNumber() async {
    try {
      contactUserInfo = await Navigator.push(
          context, MaterialPageRoute(builder: (context) => InputPhoneNumber()));
      //selectedPhoneNumber = await Navigator.pushNamed(context, InputPhoneNumberRoute);
      if (contactUserInfo.phoneNumber != null && contactUserInfo.name != null)
        Navigator.pop(context, contactUserInfo);
    } catch (Exception) {
  ...
  saveFriend() async {
    contactUserInfo = await Navigator.push(
        context, MaterialPageRoute(builder: (context) => SaveFriend()));
  ...
  searchPhoneNumber(Contact _c) async {
    if (SoundSearcher.matchString(
        _c.displayName, _textEditingController.text)) {
      return true;
    } else
      return false;
  }
